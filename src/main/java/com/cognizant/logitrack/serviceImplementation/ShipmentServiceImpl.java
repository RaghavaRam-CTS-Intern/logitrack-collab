...
public class ShipmentServiceImpl implements ShipmentService {
    ...
    public ShipmentDTO createShipment(ShipmentDTO dto) {
        ...
        boolean pickListCompleted = pickListRepository
                .findByFreightOrderId(freightOrder.getFreightOrderId())
                .stream()
                .anyMatch(pickList -> pickList.getStatus() == PickListStatus.COMPLETED);

        if (!pickListCompleted) {
            throw new BadRequestException(
                    "Cannot create shipment because pick list is not completed for freight order: "
        ...
        Carrier carrier = carrierRepository.findById(dto.getCarrierId())
                .orElseThrow(() -> new BadRequestException(
                        "Carrier not found: " + dto.getCarrierId()
                ));
        ...
        LocalDate shipmentDate = dto.getDispatchDate() != null
                ? dto.getDispatchDate()
                : LocalDate.now();
        ...
        log.info(
                "Shipment created: id={}, freightOrderId={}, carrierId={}, rateCardId={}, freightCost={}",
                saved.getShipmentId(),
                freightOrder.getFreightOrderId(),
                carrier.getCarrierId(),
    ...
    public List<DeliveryEventDTO> getEventsByShipment(Integer shipmentId) {
        return deliveryEventRepository.findByShipment_ShipmentId(shipmentId)
                .stream()
                .map(this::toEventDTO)
        ...
        return ShipmentDTO.builder()
                ...
                .freightOrderId(
                        shipment.getFreightOrder() != null
                                ? shipment.getFreightOrder().getFreightOrderId()
                                : null
                )
                .carrierId(
                ...
                .rateCardId(
                        shipment.getRateCard() != null
                                ? shipment.getRateCard().getRateCardId()
                                : null
                )
    ...
    private void validateWeightSlab(BigDecimal weight, String weightSlab) {
        ...
        if (weightSlab == null || weightSlab.isBlank()) {
            throw new BadRequestException("Rate card weight slab cannot be empty");
        }

        String cleanedSlab = weightSlab
                .toLowerCase()
                .replace("kg", "")
                .replace(" ", "");

        String[] parts = cleanedSlab.split("-");

        if (parts.length != 2) {
            throw new BadRequestException("Invalid weight slab format: " + weightSlab);
        }

        BigDecimal minWeight = new BigDecimal(parts[0]);
        BigDecimal maxWeight = new BigDecimal(parts[1]);

        if (weight.compareTo(minWeight) < 0 || weight.compareTo(maxWeight) > 0) {
            throw new BadRequestException(
                    "Freight order weight " + weight
                            + " is not within rate card weight slab " + weightSlab
            );
        }
    }
        ...
        return DeliveryEventDTO.builder()
                ...
                .shipmentId(
                        event.getShipment() != null
                                ? event.getShipment().getShipmentId()
                                : null
                )
