 ...
 * audit write hook itself, so all three are excluded here to avoid duplicate or
 * recursive entries.
 */
@Aspect
@Component
...
public class AuditAspect {
	...
	public void auditWrite(JoinPoint joinPoint) {
		try {
			...
			AuditLog auditLog = AuditLog.builder().user(actor).action(action).entityType(entityType).build();
			auditLogRepository.save(auditLog);
			log.debug("Audit recorded: action={}, entityType={}, userId={}", action, entityType,
